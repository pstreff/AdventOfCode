...
ore = defaultdict(int)
amount_of_ore_available = 1000000000000
fuel = 2370000  # estimated

while ore['ORE'] <= amount_of_ore_available:
    print(ore['ORE'], fuel)
    ore.clear()
    storage.clear()
    calculate_ores('FUEL', fuel)
    if ore['ORE'] <= amount_of_ore_available:
        fuel += 1
    else:
        break

print('We can produce {} FUEL'.format(fuel - 1))  # don't know why this -1 but it works
